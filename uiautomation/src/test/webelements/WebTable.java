...
public class WebTable extends WebTypifiedElement {
	 ...
	 * <th>}).
	 * <p>
	 * Multiple rows of heading elements, ({@code 
	 * <tr>
	 * }), are flattened i.e. the second row, ({@code 
	 * <tr>
	 * }), will follow the first, which can be misleading when the table uses
	...
	/**
	 * Returns table cell elements ({@code 
	 * <td>}), grouped by rows.
	 *
	...
	public List<List<WebElement>> getRows() {
		return getWrappedElement().findElements(By.xpath(".//tr")).stream()
				.map(rowElement -> rowElement.findElements(By.xpath(".//td"))).filter(row -> row.size() > 0) // ignore
																												// rows
																												// with
																												// no
																												// <td>
	...
	/**
	 * Returns text values of table cell elements ({@code 
	 * <td>}), grouped by rows.
	 *
	 * @return List where each item is text values of a table row.
	 */
	public List<List<String>> getRowsAsString() {
		return getRows().stream().map(row -> row.stream().map(WebElement::getText).collect(toList())).collect(toList());
	}
	...
	public List<WebElement> getColumnByIndex(int index) {
		return getWrappedElement().findElements(By.cssSelector(String.format("tr > td:nth-of-type(%d)", index)));
	}
	...
	public List<List<String>> getColumnsAsString() {
		return getColumns().stream().map(row -> row.stream().map(WebElement::getText).collect(toList()))
				.collect(toList());
	}

	/**
	 * Returns table cell element ({@code 
	...
	/**
	 * Returns list of maps where keys are table headings and values are table row
	 * elements ({@code 
	 * <td>}).
	 */
	public List<Map<String, WebElement>> getRowsMappedToHeadings() {
		...
		return getRows().stream()
				.map(row -> row.stream().collect(toMap(e -> headingsAsString.get(row.indexOf(e)), identity())))
				.collect(toList());
	}

	/**
	 * Returns list of maps where keys are passed headings and values are table row
	 * elements ({@code 
	 * <td>}),.
	 *
	...
	public List<Map<String, WebElement>> getRowsMappedToHeadings(List<String> headings) {
		return getRowsMappedToHeadings().stream().map(e -> e.entrySet().stream()
				.filter(m -> headings.contains(m.getKey())).collect(toMap(Map.Entry::getKey, Map.Entry::getValue)))
				.collect(toList());
	}
	...
	public List<Map<String, String>> getRowsAsStringMappedToHeadings() {
		return getRowsMappedToHeadings().stream()
				.map(m -> m.entrySet().stream().collect(toMap(Map.Entry::getKey, e -> e.getValue().getText())))
				.collect(toList());

	}

	/**
	 * Same as {@link #getRowsMappedToHeadings(java.util.List)} but retrieves text
	 * from row elements ({@code 
	 * <td>}).
	 */
